...
class HDF5Handler:
    ...
    def _addDataByArray(self, Chrom, resolution, data_name, value_array, compression='lzf'):
        """ Add array to the hdf5 file for given chromosome, resolution and coarse method.

            It should be called when new data array is added to hdf5 file.
        ...
        resolution : str
            Reslution of data
        data_name : str
            Name of data coarsing method
        value_array : numpy.ndarray
            Array containing values where its index is real location on chromosome.
...
class BigWigHandler:
    ...
    def __init__(self, filenames, pathTobigWigToWig, pathTobigWigInfo, resolutions=None, methodToCombine='mean', workDir=None, maxEntryWrite=10000000):
    ...
    def _bigWigtoWig(self, bigWigFileName, outfilename, ChrNum):
        ...
        ChrNum : str
            Only convert wig format for this chromosome. For all chromosomes, use: ``None``.
        ...
        if ChrNum is None:
            ...
            cmd = '{0} -chrom=chr{1} {2} {3}' .format(self.pathTobigWigToWig, ChrNum, bigWigFileName, outfilename)
    ...
    def bigWigtoWig(self, outfilenames=None, ChrNum=None):
        ...
        ChrNum  : str (optional)
            Only convert wig format for this chromosome. For all chromosomes, use ``None`` or leave it.
        ...
        for i in range(len(self.bigWigFileNames)):
            self._bigWigtoWig(self.bigWigFileNames[i], self.WigFileNames[i], ChrNum)

    def saveToH5(self, filename, tmpNumpyArrayFiles=None, title=None, compression='lzf'):
        ...
        self.wigHandle.convertWigToH5(filename, title=title, compression=compression)
...
class WigHandler:
    ...
    """
    def __init__(self, filenames, chromSizeInfo, resolutions=None, tmpNumpyArrayFiles=None, methodToCombine='mean', workDir=None, maxEntryWrite=10000000):
    ...
    def _StoreInHdf5File(self, hdf5Out, title, compression):
        ...
        for Chrom in self.chromSizeInfo:
            for res in self.resolutions:
                hdf5Out._addDataByArray(Chrom, '1b', 'Original', self.tmpNumpyArrayFiles.arrays[Chrom][:], compression='gzip')
                for coarsing_method in ['min', 'max', 'amean', 'hmean', 'gmean', 'median']:
                    hdf5Out._addDataByArray(Chrom, res, coarsing_method, self._PerformDataCoarsing(Chrom, res, coarsing_method), compression=compression )
    ...
    def convertWigToH5(self, hdf5Out, title=None, compression='lzf'):
        """To convert Wig files to hdf5 file
        ...
        self._StoreInHdf5File(hdf5Out, title, compression)
